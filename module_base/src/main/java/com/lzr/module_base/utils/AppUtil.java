...
public class AppUtil {
    ...
    public static boolean isFirstRun(Context context) {
        ...
        if (versionCode != newAppVersion) {
            sp.edit().putInt("versionCode", newAppVersion).apply();
            isFirstRun = true;
        }
        return isFirstRun;
    }
...
//    }
    ...
    public static boolean isShowNavigationBar(Activity activity){
        ...
        if (AppUtil.getScreenRealHeight(activity) - getNavigationBarHeight(activity)
                == contentView.getHeight()) {
            //显示
//            Log.e(TAG, "onLayoutChange: NavigationBar显示" );
            return true;
        } else {
            //隐藏
//            Log.e(TAG, "onLayoutChange: NavigationBar隐藏" );
            return false;
     ...
     * 获取屏幕高度
     *
     * getMetrics与getRealMetrics相差了一个getNavigationBarHeight的高度
     * @param context
     * @return
     */
     ...
     * 判断是否有底部导航栏,
     * 注意：不能用来判断是否显示
     * 原理：如果存在底部导航栏，realDisplayMetrics和displayMetrics相差一个底部导航栏的高度
     * @param activity
     * @return
     */
    ...
    public static void createOnlyShortcut(Context context, int appNameId, int ic_launcherId, Class launcherActivity) {
        ...
        if ((boolean)SPUtils.get("isFirst", true)) { //是第一次启动应用
